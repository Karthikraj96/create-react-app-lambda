...
import { decodedata, addCourseRppFee, getAllOrgRppFee, getAllYear } from '../../api/api';
import { faPencil, faEye, faRupeeSign } from '@fortawesome/pro-duotone-svg-icons';
import RPPSettingsModal from './RPPSettingsModal';
import RPPSettingsProfile from './RPPSettingsProfileDrawer';
...
function RPPSetings() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <>
            <FontAwesomeIcon icon={faRupeeSign} /> {' ' + record.feesRenewal.toLocaleString('en-IN')}/-
          </>
        );
      },
      key: 'fees',
  ...
  return (
    <Fragment>
      ...
      />
      <RPPSettingsProfile org={org} record={record2} isVisible={isProfileDrawer} handleCancel={toggleProfile} />
