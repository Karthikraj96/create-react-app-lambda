...
function TwoPlusSettingsModal({ isVisible, handleOk, tokendata, handleCancel, year, grade, org, record, setRecord }) {
  useEffect(() => {}, [record]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item label="Select Grade" name="grade_id" rules={[{ required: true, message: 'Please enter the Grade!' }]}>
          <Select
            style={{ width: '100%', height: '40px', padding: '5px' }}
            onChange={value => {
        ...
        <Form.Item label=" Two Plus Fee" name="fees" rules={[{ required: true, message: 'Please enter the  Two Plus Fee!' }]}>
          <InputNumber
            onChange={value => {
              onChange(value, 'fees');
            }}
...
export default TwoPlusSettingsModal;
